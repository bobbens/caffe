...
DataTransformer<Dtype>::DataTransformer(const TransformationParameter& param,
  ...
  CHECK(param_.crop_size() == 0 ||
       (param_.crop_size_h() == 0 && param_.crop_size_w() == 0))
    << "Crop size is crop_size OR crop_h and crop_w; not both";
  CHECK((param_.crop_size_h() != 0) == (param_.crop_size_w() != 0))
...
vector<int> DataTransformer<Dtype>::InferBlobShape(const cv::Mat& cv_img) {
  ...
  shape[2] = (crop_size_h)? crop_size_w: img_height;
  shape[3] = (crop_size_h)? crop_size_w: img_width;
  return shape;
}
